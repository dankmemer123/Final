package com.example.vishnunair.ftutoria.serve;
...
import com.example.vishnunair.ftutoria.Loggin;
import com.example.vishnunair.ftutoria.Messaging;
import com.example.vishnunair.ftutoria.R;
import com.example.vishnunair.ftutoria.toolbox.HandlerXML;
...
import com.example.vishnunair.ftutoria.interfacer.SocketerInterface;
import com.example.vishnunair.ftutoria.interfacer.Updater;
import com.example.vishnunair.ftutoria.comm.Socketer;
import com.example.vishnunair.ftutoria.toolbox.ControllerOfFriend;
import com.example.vishnunair.ftutoria.toolbox.ControllerOfMessage;

/**
 * This is an example of implementing an application service that runs locally
 * in the same process as the application.  The {LocalServiceController}
 * and { LocalServiceBinding} classes show how to interact with the
 * service.
 *
...
*/
    ...
    public String authenticateUser(String usernameText, String passwordText) throws UnsupportedEncodingException
    {
        ...
        if (result != null && !result.equals(Loggin.AUTHENTICATION_FAILED))
        {
            ...
            i.putExtra(InfoOfFriends.FRIEND_LIST, rawFriendList);
            sendBroadcast(i);
                ...
                public void run()
                {
                    try {
                        ...
                        if (tmp != null) {
                            ...
                            sendBroadcast(i);
                            Log.i("frnd list brdct sent ", "");

                            if (tmp2 != null) {
                                i2.putExtra(InfoOfMessage.MESSAGE_LIST, tmp2);
                                sendBroadcast(i2);
                                Log.i("frd lst brdcst sent ", "");
                            }
                        }
                        else {
                            Log.i("friend list r null", "");
                        }
                    }
                    catch (Exception e) {
                        e.printStackTrace();
